...
fadeTween = noone; //oLinearNumberTween
...
function added() {
	...
	if(self.fadeInDuration > 0) {
		self.image_alpha = 0;
		self.fadeTween = instance_create_depth(0,0,0,oLinearNumberTween);
		self.fadeTween.LinearNumberTween(self.id, "image_alpha", 1, self.fadeInDuration, method(self.id, self.fadeInCallback));
		self.fadeIn();
	}
	else {
		self.image_alpha = 1;
...
function fadeOut() {
	if(self.fadeOutDuration > 0) {
		self.fadeTween = instance_create_depth(0,0,0,oLinearNumberTween);
		self.fadeTween.LinearNumberTween(self.id, "image_alpha", 0, self.fadeOutDuration, noone, true);
	}
	else {
